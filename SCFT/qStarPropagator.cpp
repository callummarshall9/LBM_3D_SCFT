...
qStarPropagator::qStarPropagator(int nx, int ny, int nz, std::string velocity_set, double c_s, std::string boundary_condition, double gamma_dot, double N, int N_s, double box_length_rg, std::string field_type, double f, double chiN, double* w_A, double* w_B):
        LBM(nx,ny,nz,velocity_set,c_s,boundary_condition,gamma_dot,N,N_s,box_length_rg,field_type,f,chiN,w_A,w_B) {
}

double qStarPropagator::field(int x, int y, int z) {
