...
public:
	LBM(int grid_size, std::string velocity_set, double c_s, double tau, std::string boundary_conditions, double gamma_dot, int runs);
	LBM(int nx, int ny, int nz, std::string velocity_set, double c_s, double tau, std::string boundary_condition, double gamma_dot, int runs);
	~LBM();
	...
	void output_test();
	void set_field_paramaeters(double N, double Length_Rg, std::string field_type);
	...
	void output_lbm_data(std::string filename, bool header=true, bool output_indices = false);
	double find_max();//Used to find the max value.
	double find_min();//Used to find the min value.
	void lookup_reverse();
	double field(int x, int y, int z);
...
private:
	...
	double* particle_distributions_star;
